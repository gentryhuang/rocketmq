...
public class MixAll {
    ...
    /**
     * 死信队列的 Topic 名：
     * 1 Broker中单独的一个队列(DLQ)，该队列存储了Consumer端重试16次后都没成功消费的消息
     * 2 该队列：只有写权限，没有读权限。所以是不能被Consumer重新消费的，只能进行人工干预，重新投递（Rocket-MQ-Console 中可以操作）
     * 3 DLQ队列中，该消息的TOPIC 重新被命名为： "%DLQ%" + groupName
     * 4 DLQ队列其实就是（consumequeue文件夹的"%DLQ%" + groupName 命名的Topic文件夹下的队列）
     */
