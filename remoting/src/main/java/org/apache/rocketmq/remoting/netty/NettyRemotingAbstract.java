...
/**
 * 远程通信抽象类，通信的服务端和客户端都要继承该类。
 * 1 提供统一处理入口 processMessageReceived 方法，供服务端和客户端使用。该方法中会根据是请求还是响应做不同的处理。
 * 2 该抽象类聚合了 NettyRequestProcessor 处理器和对应的处理线程池
 * todo 特别说明：
 * 1 该类用于为客户端或服务端提供基本的请求或响应流程
 * 2 该类用于为客户端或服务端绑定对应处理器 NettyRequestProcessor，这样的话，当客户端或服务端收到消息或发送消息（通过对 Netty 的封装），就可以将数据
 * 交给对应的本次请求码对应的 NettyRequestProcessor 处理，具体处理是交给它关联的线程池。
 */
    ...
    public void processRequestCommand(final ChannelHandlerContext ctx, final RemotingCommand cmd) {
        ...
        if (pair != null) {
            ...
            try {
                ...
                // 如果是非 oneway 的请求，本来需要响应的，但此时无法处理过多请求，需要给出对应的错误提醒
                if (!cmd.isOnewayRPC()) {
                    final RemotingCommand response = RemotingCommand.createResponseCommand(RemotingSysResponseCode.SYSTEM_BUSY,
