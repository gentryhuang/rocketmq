...
public class ConsumerOffsetManager extends ConfigManager {
    ...
    /**
     * 提交消费进度
     * todo 1 可以看到，每次Consumer进行RPC调用上报自己的消费进度，Broker接收之后并没有立即进行持久化，而是直接更新到内存中。
     * todo 2 具体的持久化到文件是通过 Broker 启动时开始的定期任务
     *
    ...
    public void decode(String jsonString) {
        if (jsonString != null) {
            // 获取文件内容后，序列化为ConsumerOffsetManager对象
            // 实质是其属性ConcurrentMap<String，ConcurrentMap<Integer, Long>> offsetTable
